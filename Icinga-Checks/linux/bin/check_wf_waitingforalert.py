...
Overview & Goal
----------------------
Check for workflows against a target Mongo database collection for select workflow names that are in status 'waiting for alert'. 
This means the workflow has ran into an error and  will never resolve without human interaction. 
...
examples:
$python3 check_wf_waitingforalert.py --dbuser USER1 --dbpassword USER1PASS --ipaddress 0.0.0.0 --database RECOVER --collection WFINSTDATA --workflows "WF1;WF2;WF99"
...
def ExecuteQuery(dbuser, dbpass, dbipaddress, database, collection, workflows):
    ...
    filter = {"Status":"WaitingForAlert", "DefinitionId":{"$in":workflows.split(';')}}
...
def main(dbuser, dbpass, ipaddress, database, collection, workflows):
    ...
    expected_issue_response = "Workflows waiting for alert have been detected. Please clean up and correct underlying issue." 
